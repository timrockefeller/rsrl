...
fn player_speed_modify(
    keys: Res<ButtonInput<KeyCode>>,
    key_bindings: Res<KeyBindings>,
    mut movement_settings: ResMut<MovementSettings>,
) {
    if keys.just_pressed(key_bindings.speed_up) && movement_settings.speed < 120f32 {
        movement_settings.speed += 4f32;
    }
    if keys.just_pressed(key_bindings.speed_down) && movement_settings.speed > 4f32 {
        movement_settings.speed -= 4f32;
    }
}
